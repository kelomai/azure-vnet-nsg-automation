...
foreach ($resourceGroup in $resourceGroups.Keys) {
    ...
    if ($rgExists -eq "true") {
        ...
        if ($confirmDelete -match "^(yes|y)$") {
            ...
            Write-Host -ForegroundColor Yellow "⏭️  Deletion of resource group '$resourceGroup' canceled by user."
        }
    }
    else {
        Write-Host -ForegroundColor Yellow "⏭️  Resource group '$resourceGroup' does not exist. Skipping."
    }
